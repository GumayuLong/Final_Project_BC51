...
const items = [
  {
    label: <a href="/login">Đăng ký</a>,
    key: "0",
  },
  {
    label: <a href="/register">Đăng nhập</a>,
    key: "1",
  },
  {
    type: "divider",
...
export default function Header() {
  ...
  const renderLocateDetail = () => {
    return locateDetail.map((element) => {
      return (
        <Option key={element.id} value={element.id} label={element.tinhThanh}>
          <Space>
            <span role="img"></span>
            {element.tinhThanh}
          </Space>
  ...
  const renderContentRight = () => {
    if (!userState.userInfo) {
      ...
      <>
        <span>HI~ {userState.userInfo.name}</span>{" "}
    ...
    <nav className="navbar navbar-expand-sm navbar-light bg-light py-3 px-3">
      <a className="navbar-brand" href="/">
        <svg
          ...
          height={32}
          fill="currentcolor"
          style={{ display: "block", color: "#f43f7b" }}
        >
      ...
      <div className="header-rear">
        <Space>
          <Button type="text">Trở thành chủ nhà</Button>
          <Button type="text">
            <GlobalOutlined />
          </Button>
          {renderContentRight()}
